	...
	go func() {
		...
		for {
			...
			case event := <-w.fswatcher.Events:
				...
				_, keypairChanged := keypairMap[path]
				_, caChanged := caMap[path]
				createOperation := event.Op&fsnotify.Create == fsnotify.Create
				if (keypairChanged || caChanged) && createOperation {
	...
	add := func(path string) error {
		...
		if err := fswatcher.Add(path); err != nil {
			...
			if err := fswatcher.Add(dir); err != nil {
				...
				// the file itself.
				return fmt.Errorf("Failed to add %q to fsnotify watcher: %s", dir, err)
			}
		}
		return nil
	}
